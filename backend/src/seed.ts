...
async function bootstrap() {
  ...
  let existing = await userRepository.findOne({ where: { email: 'mock@example.com' } });
  ...
  const payload = { sub: existing.id, email: existing.email, name: existing.name };
  const token = jwtService.sign(payload, { secret: 'super_secret_jwt_key_here' });
  console.log('\n=======================================');
